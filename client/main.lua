...
Citizen.CreateThread(function()
	while true do
		...
		local DetectableTextures = {
            ...
            {txd = "FiveSense", txt="fivesense", name="Five Sense"},
            {txd = "iPhone", txt="iphone_menu", name="iPhone Menu"},
            {txd = "AquInspired", txt="aqu441", name="Aqu Inspired Cheat"},
		...
		for i, data in pairs(DetectableTextures) do
			if data.x and data.y then
				if GetTextureResolution(data.txd, data.txt).x == data.x and GetTextureResolution(data.txd, data.txt).y == data.y then
					TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Lua Menu detected: " ..data.name, "basic", true)
				end
			else 
        ...
        if data.text ~= nil then     
            for _, word in pairs(clientconfig.Menu.BLWordsOnScreen) do
                if string.find(string.lower(data.text), string.lower(word)) then
                    TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Blacklisted Word on screen detected: " ..word, "basic", true)
                end
            end
        end
        ischecking = false
...
Citizen.CreateThread(function()
    if clientconfig.screen.Freecamcheck then
        ...
        if (camcoords.x > 9) or (camcoords.y > 9) or (camcoords.z > 9) or (camcoords.x < -9) or (camcoords.y < -9) or (camcoords.z < -9) then
            TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Freecam detected", "basic", true)
        end
        Citizen.Wait(350)
    end
end)
        ...
        while true do
            ...
            for RE, RF in ipairs(GetRegisteredCommands()) do
                for RG, RCO in ipairs(clientconfig.Menu.BLCommands) do
                    if RF.name == RCO then
                        TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Blacklisted Command detected: " ..RF.name, "basic", true)
                    end
                end
            end
        end
        ...
        while true do
            ...
            if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                if RI == RY and RJ ~= RZ and RZ ~= nil and RZ ~= 0 then
                    DeleteVehicle(RI)
                    local src = source
                    TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "CheatEngine Detected ", "basic", true)
                    return
                end
            end
            RY = RI
...
RegisterNetEvent('ftBWDu9IGvSrh8ZSiFAJnaIv6', function(nekruc, violator)
...
function Scaleforms()
	...
	TriggerServerEvent('ftBWDu9IGvSrh8ZSiFAJnaIv6', coords, pedid)
	canDraw = 5000
...
--[[if clientconfig.ingame.AntiRandomStringResource then
    if not clientconfig.whitelistedanti[k] then
        ...
        function(res)
            ...
            if a >= 18 then
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Random string resource detected: " ..res, "basic", true)
            end
        end
        )
    end
...
RegisterNetEvent("ZDqqUXKA3Jj9xO3qgVqv2rUJm")
AddEventHandler("ZDqqUXKA3Jj9xO3qgVqv2rUJm", function()
		...
		while true do
			...
			local playerlist = GetActivePlayers()
				for i = 1, #playerlist do
					...
					if blipcount > 0 then
						TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Player blips Detected", "basic", true)
					end
				end
		end
	end)
    ...
    while true do 
        ...
        if clientconfig.player.AntiInfiniteStamina then
            if GetEntitySpeed(_ped) > 7 and not IsPedInAnyVehicle(_ped, true) and not IsPedFalling(_ped) and not IsPedInParachuteFreeFall(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedRagdoll(_ped) then
                ...
                if tonumber(_staminalevel) == tonumber(0.0) then
                    TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Infinity Stamina detected", "basic", true)
                end
            end
        end
        Citizen.Wait(1000)
        if clientconfig.player.AntinonRagdoll then
            if not CanPedRagdoll(_ped) and not IsPedInAnyVehicle(_ped, true) and not IsEntityDead(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedJacking(_ped) then
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Anti Ragdoll detected", "basic", true)
            end
            Citizen.Wait(5000)
        end
        Citizen.Wait(1000)
        if clientconfig.player.AntiExplosiveBullet then
            ...
            if _weapondamage == 4 or _weapondamage == 5 or _weapondamage == 6 or _weapondamage == 13 then
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Explosive Weapon detected", "basic", true)
            end
            Citizen.Wait(200)
        end
        if clientconfig.player.AntiInvisble then
            ...
            if not IsEntityVisible(_ped) or not IsEntityVisibleToScript(_ped) or _entityalpha <= 150 then
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Player is Invisible", "basic", true)
            end
            Citizen.Wait(200)
        end
    end
    ...
    Citizen.CreateThread(function()
        while true do
         ...
         if GetUsingnightvision() then
             TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "NightVision detected", "basic", true)
         end
        end      
    end)    
end    
...
AddEventHandler('onClientResourceStop', function(resourceName)
    if clientconfig.player.AntiScriptStop then
        TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Script stop detected: " ..resourceName, "basic", true)
    end    
end)        
		...
		while true do
			...
			for _,g in ipairs(clientconfig.Weapons.BLWeapons) do
				...
				if HasPedGotWeapon(PlayerPedId(),GetHashKey(g),false) == 1 then
                    ...
                    Citizen.Wait(1000)
                    TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Blacklisted Weapon detected: " ..g, "model", true)
				end
			end
		end
	end)
                ...
                for _, BlockedPlate in pairs(clientconfig.blacklistedplates.blacklistedplates) do 
                    if plate == BlockedPlate then
                        if DriverSeat == ped then
                            DeleteVehicle(veh)
                            TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Blacklisted Plate detected: " ..BlockedPlate, "basic", true)
                        end
                    end
                end
            end
...
Citizen.CreateThread(function() -- Antigodmode / Nic
    if clientconfig.player.AntiGodMode then
        if GetEntityHealth(PlayerPedId()) > clientconfig.player.Maxplayerhealth then
            TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "More than" ..clientconfig.player.Maxplayerhealth.. " health detected", "basic", true)
        end
        if not GetPlayerInvincible(PlayerId()) then
        else
          TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "GodMode detected", "basic", true)
    ...
    if clientconfig.player.AntiArmor then
        ...
        local armor = GetPedArmour(ped)
         ...
         if armor > clientconfig.player.MaxArmor then
               SetPedArmour(ped, 0)
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "More than " ..clientconfig.player.MaxArmor.. " Armour detected", "basic", true)
         end  
...
if clientconfig.screen.AntiNUIDEVTools then -- AntiNui devtools / Nic
    RegisterNUICallback("callback", function()
      ...
      AddEventHandler("onResourceStart", function(a)
        TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Extended Anti NUI devtool detected", "basic", true)
      end)
    end)
end
...
if clientconfig.screen.AntiThermalVision then
    if GetUsingnightvision() then
        TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "ThermalVision detected", "basic", true)
    end 
end   
	...
	while true do
        ...
        if clientconfig.player.AntiSuperJump then
		    if IsPedJumping(PlayerPedId()) then
			    ...
			    until not h
			    if m > 250 then
			    	TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "SuperJump detected", "basic", true)
			    end
		    end
	    end
    end    
	...
	if clientconfig.player.AntiSpectate then
		while true do
			...
			if NetworkIsInSpectatorMode() then
    			TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Spectate detected", "basic", true)
    		end
		end
	end
end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if GetDistanceBetweenCoords(posx,posy,posz, newx,newy,newz) > 1 and still == IsPedStill(ped) and vel == GetEntitySpeed(ped) and ped == newPed then
                TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Changed Position too quick (NoClip)", "basic", true)
            end
        end
    end)
end
...
local function collectAndSendResourceList()
	...
	Wait(10)
    TriggerServerEvent("XEcMMBfsyLnn1PxyOwX1Pz3hi", resourceList)
end
...
function checkCar(car)
	if car then
		...
		if isCarBlacklisted(carModel) then
			DeleteEntity(car)
			TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Blacklisted Car detected: " ..carName, "basic", true)
		end
	end
end
		...
		for number in EnumerateVehicles() do
			for Q3, models in pairs(clientconfig.Cars.BLCars) do
				if IsVehicleModel(number, models) then
					...
					TriggerServerEvent("CCjBvpCZj2ObDucT9WbR37eXo", "Tried to spawn: " ..models, "basic", true)
                    end
				end
			end
		end
